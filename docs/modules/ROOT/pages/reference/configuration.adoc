...
=== `embedding` and `llm` — three accepted shapes

As of v0.3 both fields accept a discriminated union:

* **legacy `EmbeddingConfig` / `LLMConfig`** — emits one `DeprecationWarning` per construction when user-explicit; planned removal in v0.5.0. Backward compat preserved through v0.4.
* **provider-string shorthand** — e.g. `"openai/text-embedding-3-small"` or `"anthropic/claude-3-5-sonnet-latest"`. Resolved via `neo4j_agent_memory.llm.from_provider`.
* **fully-constructed Provider instance** — the new canonical shape. See xref:reference/api/adapters.adoc[Adapters Reference].
...
# Explicit Provider instance (full control)
from neo4j_agent_memory.llm.adapters.anthropic import AnthropicProvider

settings = MemorySettings(
    neo4j={"password": "p"},
    llm=AnthropicProvider("anthropic/claude-3-5-sonnet-latest", cache_system=True),
)

# Legacy (still works, one DeprecationWarning per construction)
...
For the full migration guide, see xref:how-to/migrate-to-providers.adoc[Migrate to Pluggable Providers].
...
schema_config = SchemaConfig(
    ...
    skip_subsystems=frozenset(),           # Subsystems to leave uninstalled
)
----

`skip_subsystems` names the memory subsystems whose constraints and
indexes are not created on connect. The default installs the full schema.
See xref:reference/schema-objects.adoc[Schema Objects] for the subsystem
inventory and what a skip costs.
...
NAM_SCHEMA_CONFIG__SKIP_SUBSYSTEMS='["facts","geospatial"]'
----
